...
配置 sentinel.conf 文件
```shell
# 从 redis 中复制一份 sentinel.config 文件出来
cp sentinel.config sentinel_26379.conf
# 编辑 sentinel_26379.config 文件
...
pidfile /var/run/redis-sentinel-26379.pid # 设置保存redis进程id的文件名称
sentinel monitor mymaster 10.211.55.3 26379 2  # 指定主节点ip、port，2表示如果有三台服务器，必须要超过两台认为 master 失效，才是真的失效
dir /root/Dev_Azh/redis/data/26379  # 指定工作运行空间，log存储目录
logfile "sentinel-26379.log" # 日志名称
```
注意：其余两个 ip 要指定 master 的 ip 地址以及 port 端口号。工作目录以及日志名称
配置完成后启动 redis 实例：
```shell
# 启动第一个实例
./redis-sentinel ../../config/sentinel/sentinel_26379.conf
./redis-sentinel ../../config/sentinel/sentinel_26380.conf 

```
启动成功后，在主节点的 sentinel 配置文件中，可以看到如内容：
```shell
protected-mode no
sentinel known-sentinel mymaster 10.211.55.5 26381 c6e1541643b556b64f77c07e6b3f8629a46c20c9
sentinel known-sentinel mymaster 10.211.55.3 26379 2fec21dedec683222a363f13a98470726f3e72b7
sentinel known-sentinel mymaster 10.211.55.4 26380 077ce323a9af14727a221fa19fa33c47db127f32
sentinel current-epoch 0
```
连接上客户端可以通过命令查看是否搭建成功：
```shell
# 连接客户端
./redis-cli -p 26380
# 查看当前集群的主节点
sentinel masters # 可以看到对应的 master 主节点名称，对应的ip、port
```
